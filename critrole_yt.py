...
def parse_critrole_title(title):
    match = re.match(r".*(Critical Role):?.* (Campaign (\d+):?),?,? Episode ?(\d+)", title, flags=re.I)
    ...
    if match:
        campaign = "1"
        if match.group(2):
          print("got episode and campaign!")
...
def main():
    ...
    feedUrl = 'https://www.youtube.com/feeds/videos.xml?channel_id=UCpXBGqwsBkpvcYjsJBQ7LEQ'
